...
# VERIFICAÇÕES
# ═══════════════════════════════════════════════════════════
[[ "$(uname)" != "Linux" ]] && fail "Open-PY requer Linux (Ubuntu 22.04+ / Debian 12+)"
[[ "$EUID" -ne 0 ]] && fail "Execute como root: curl ... | sudo bash"
...
if ! dpkg -s "postgresql-${PG_VERSION}-pgvector" &>/dev/null 2>&1; then
    ...
    apt-get install -y -qq "postgresql-${PG_VERSION}-pgvector" >> "$LOG_FILE" 2>&1 || {
        warn "Compilando pgvector do fonte..."
        cd /tmp && git clone --branch v0.7.4 https://github.com/pgvector/pgvector.git >> "$LOG_FILE" 2>&1
        cd pgvector && make >> "$LOG_FILE" 2>&1 && make install >> "$LOG_FILE" 2>&1
...
if [[ -d "$INSTALL_DIR/.git" ]]; then
    ...
    if [[ -d "$INSTALL_DIR" ]]; then
        ...
        [[ -f "$CONFIG_FILE" ]] && cp "$CONFIG_FILE" /tmp/openpy.toml.bak
        rm -rf "$INSTALL_DIR"
    fi
    step "Clonando repositório..."
    git clone "$REPO_URL" "$INSTALL_DIR" >> "$LOG_FILE" 2>&1
    # Restaurar config se existia
...
sudo -u postgres psql -c "CREATE USER openpy WITH PASSWORD '$DB_PASSWORD';" >> "$LOG_FILE" 2>&1
sudo -u postgres psql -c "CREATE DATABASE openpy OWNER openpy;" >> "$LOG_FILE" 2>&1
sudo -u postgres psql -d openpy -c "CREATE EXTENSION IF NOT EXISTS vector;" >> "$LOG_FILE" 2>&1
sudo -u postgres psql -d openpy -c "CREATE EXTENSION IF NOT EXISTS pg_trgm;" >> "$LOG_FILE" 2>&1

ok "User: openpy | Database: openpy | Extensions: pgvector, pg_trgm"
...
pip install -r "$INSTALL_DIR/requirements.txt" -q >> "$LOG_FILE" 2>&1 || {
    warn "Instalando pacotes individualmente..."
...
for p in "${PROVIDERS[@]}"; do
    ...
    case "$p" in
        1)
            echo -ne "\n  ${CYAN}OpenAI API Key: ${NC}"; read -rs OPENAI_KEY; echo ""
            OPENAI_ENABLED=true
            [[ -z "$DEFAULT_MODEL" ]] && DEFAULT_MODEL="gpt-4o-mini"
            ok "OpenAI ativado"
            ;;
        2)
            echo -ne "\n  ${CYAN}Anthropic API Key: ${NC}"; read -rs ANTHROPIC_KEY; echo ""
            ANTHROPIC_ENABLED=true
            [[ -z "$DEFAULT_MODEL" ]] && DEFAULT_MODEL="claude-sonnet-4-20250514"
            ok "Anthropic ativado"
            ;;
        3)
            echo -ne "\n  ${CYAN}OpenRouter API Key: ${NC}"; read -rs OPENROUTER_KEY; echo ""
            OPENROUTER_ENABLED=true
            [[ -z "$DEFAULT_MODEL" ]] && DEFAULT_MODEL="anthropic/claude-sonnet-4"
            ok "OpenRouter ativado"
            ;;
        4)
            echo -ne "\n  ${CYAN}NVIDIA NIM API Key: ${NC}"; read -rs NVIDIA_KEY; echo ""
            ...
            ;;
        5)
            echo -ne "\n  ${CYAN}Base URL: ${NC}"; read -r OPENCODE_BASE
            echo -ne "  ${CYAN}API Key: ${NC}"; read -rs OPENCODE_KEY; echo ""
            echo -ne "  ${CYAN}Nome do modelo: ${NC}"; read -r CUSTOM_MODEL
...
echo ""
echo -ne "  ${BOLD}Token do Bot: ${NC}"; read -rs BOT_TOKEN; echo ""
echo -ne "  ${BOLD}Seu Telegram User ID: ${NC}"; read -r TELEGRAM_USER_ID
...
ok "openpy.toml gerado (permissões restritas)"
...
systemctl enable open-py >> "$LOG_FILE" 2>&1
...
case "${1:-help}" in
    ...
    logs)     journalctl -u open-py -f --no-hostname ;;
...
read -r START_NOW
if [[ "$START_NOW" == "s" || "$START_NOW" == "S" || "$START_NOW" == "y" ]]; then
